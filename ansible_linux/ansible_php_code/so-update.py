...
AnsibleArgs = "src=%s dest=/data/apps/%s/current/ checksum=yes rsync_opts=--no-motd,--exclude=.git archive=yes delete=yes" % (CodeBakUrl,Project)
...
#modify index.php,update to online
    ...
    with open(JsonUrl,'w') as file:
        ...
        file.write(JsonDump)
print "======================================"
MyAnsible.AnsibleRunner(HostList,Project,'shell','/data/apps/so/current/bin/yg/so.sh restart')
